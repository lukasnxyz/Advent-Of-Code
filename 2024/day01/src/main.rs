...
fn main() -> std::io::Result<()> {
    ...
    let mut left = ileft.clone();
    let mut right = iright.clone();
    ...
    let mut ramount = HashMap::new();
    for r in &right {
        let freq = right.iter().filter(|&&x| x == *r).count() as i32;
        ramount.insert(r, freq);
    }

    let mut total: i32 = 0;
    for l in &left {
        total += l * ramount.get(l).unwrap_or(&0);
    }
    println!("total: {}", total);
